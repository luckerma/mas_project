...
The @fig-simulation shows the simulation in action. Users are represented by black circles, vehicles by green (available), blue (in use), red squares (depleted) and the high-demand zone by the big grey circle.
## How to Run
...
- **Center:** The center initialization strategy shows the highest average user wait time because vehicles are initially concentrated in the center and users must travel farther to reach them, especially when the high-demand zone is active and far from the center.
...
- **High-Demand:** The high-demand initialization strategy has the lowest vehicle availability because the vehicles are concentrated in the high-demand zone and are used immediately by users, reducing the overall availability. However, the availability never drops below 50%, so there is no need to increase the number of vehicles.
...
- **High-Demand:** The high-demand initialization strategy shows the highest vehicle utilization as users find the vehicles directly in the high-demand zone.
